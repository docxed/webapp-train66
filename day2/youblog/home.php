...
<body>
    ...
    <div class="container my-5">
        ...
        <div class="row">
            <?php
            $sql = "SELECT * FROM post"; // ดึงข้อมูลโพสต์ทั้งหมด
            $posts = mysqli_query($conn, $sql); // ทำการ query ข้อมูล
            foreach ($posts as $post) { // วนลูปแสดงข้อมูลโพสต์ทั้งหมด
            ?>
                <div class="col-sm-12 col-md-4 col-lg-3 mb-3">
                    <div class="card h-100">
                        <div class="card-body">
                            ...
                            <div>
                                <a href="./view.php?postId=<?= $post['id']; ?>" class="btn btn-primary mb-2" role="button">
                                    รายละเอียด
                                </a>
                                <a href="./update.php?postId=<?= $post['id']; ?>" class="btn btn-secondary mb-2" role="button">
                                    แก้ไข
                                </a>
                                <!-- ส่งค่า postId ไปที่ post_actions.php โดยใช้ method GET -->
                                <a href="./post_actions.php?delete=delete&postId=<?= $post['id']; ?>" class="btn btn-outline-danger mb-2" role="button" onclick="return confirm('คุณต้องการลบใช่หรือไม่?')">ลบ</a>
                            </div>
                        </div>
                    </div>
                </div>
